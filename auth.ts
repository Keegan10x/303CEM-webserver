...
export async function login(credentials: loginConfig) {
  let sql =
    `SELECT count(id) AS count FROM accounts WHERE user="${credentials.username}";`;
  let records = await db.query(sql);
  if (!records[0].count) {
    throw new Error(`username "${credentials.username}" not found`);
  }
  ...
  if (valid === false) {
    throw new Error(`invalid password for account "${credentials.username}"`);
  }
...
export async function register(credentials: registerConfig) {
  ...
  const sql =
    `INSERT INTO accounts(user, pass) VALUES("${credentials.username}", "${credentials.password}")`;
  console.log(sql);
